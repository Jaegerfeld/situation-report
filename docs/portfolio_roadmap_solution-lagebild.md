...
- **Warum:** Es ist der einzige Mechanismus im ausgewerteten Literaturbestand, der **stille Bevorzugung sichtbar macht** — Expedite-Spuren, Blockaden und unausgesprochene Ziehreihenfolgen — und zwar aus Daten, die die Pipeline bereits erzeugt. Ohne ihn zeigt der Report Bestand, Durchsatz und Durchlaufzeit nebeneinander, rechnet sie aber nie gegeneinander. Herleitung: Vacanti, *Actionable Agile Metrics for Predictability* (2015), Kap. 9 — Analyse `Quellen/Buchanalysen/Actionable-Agile-Metrics-I_Vacanti_Analyse.md`, Kandidaten AA2 (Anzeige), AA3 (Annahmenprüfung, Ausbau von VA1) und AA5 (Bestandszeitreihe). Deckt zugleich **H2** aus der Hohpe-Analyse ab (rechnerische vs. gemessene Durchlaufzeit als Konfidenzsignal).
