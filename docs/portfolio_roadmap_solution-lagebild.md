...
### A6 · Flow-Debt-Anzeige mit Little's-Law-Annahmenprüfung · ✅ umgesetzt (10.09.2026)
- **Was:** Eine neue Metrik `flow_debt` (GUI-Haken *Flow Debt*, CLI `--metrics flow_debt`, Toleranzband über `--debt-tolerance` bzw. GUI-Feld *Flow-Debt-Toleranz*). Aus den CFD-Tagesdaten entsteht der **Bestand als Zeitreihe** (Ankünfte an der `<First>`-Grenze minus Abgänge an der `<Closed>`-Grenze); mittlerer Bestand geteilt durch Durchsatz ergibt nach Little's Law die **genäherte** mittlere Durchlaufzeit. Der Vergleich mit der **gemessenen** mittleren Durchlaufzeit ergibt drei Zustände: *nimmt Flow Debt auf* (genähert größer — einzelne Vorgänge wurden auf Kosten anderer beschleunigt), *tilgt* (genähert kleiner), *stabil* (innerhalb des Toleranzbands). Vorgeschaltet und **über** dem Urteil ausgewiesen: die Prüfung der Little's-Law-Annahmen 1 (Zugangs- gleich Abgangsrate im Fenster) und 3 (Bestand am Anfang wie am Ende). Sind sie verletzt, trägt die Kopfzeile *verdict not dependable*.
- **Warum:** Er macht **stille Bevorzugung sichtbar** — Expedite-Spuren, Blockaden und unausgesprochene Ziehreihenfolgen — und zwar aus Daten, die die Pipeline bereits erzeugt. Ein zweiter Mechanismus mit dieser Eigenschaft ist in der bisher ausgewerteten Literatur nicht aufgetaucht; systematisch dagegen geprüft wurde es nicht. Ohne ihn zeigt der Report Bestand, Durchsatz und Durchlaufzeit nebeneinander, rechnet sie aber nie gegeneinander. Herleitung: Vacanti, *Actionable Agile Metrics for Predictability* (2015), Kap. 9 — Analyse `Quellen/Buchanalysen/Actionable-Agile-Metrics-I_Vacanti_Analyse.md`, Kandidaten AA2 (Anzeige), AA3 (Annahmenprüfung, Ausbau von VA1) und AA5 (Bestandszeitreihe). Deckt zugleich **H2** aus der Hohpe-Analyse ab (rechnerische vs. gemessene Durchlaufzeit als Konfidenzsignal).
- **Wo:** `build_reports/metrics/flow_debt.py` (`wip_series`, `FlowDebtMetric`); die Grenzauflösung und die Kumulation der CFD-Reihen sind als `resolve_flow_boundaries`/`cumulative_stage_series` aus `cfd.py` herausgezogen, damit CFD-Diagramm und Flow Debt nie über die Grenzen auseinanderlaufen. Terminologie `FLOW_DEBT` in beiden Modi; GUI-Feld und Tooltip in fünf Sprachen; Projektvorlage auf `_TEMPLATE_VERSION = 5`.
- **Zwei Entwurfsentscheidungen, die begründet werden müssen:** (1) **Das Toleranzband.** Vacanti nennt für seinen dritten Zustand („roughly equal“) keine Zahl. Ohne Band wäre `genähert ≠ gemessen` für praktisch jeden Datensatz wahr und die Anzeige meldete dauerhaft Flow Debt. Voreinstellung deshalb **15 %** der gemessenen Mitte, einstellbar — großzügig genug, um bei gewöhnlicher Streuung zu schweigen. (2) **Der Durchsatz wird an der CFD-Grenze gemessen**, nicht als Zahl der Issues mit Closed Date. Bestand und Durchsatz müssen dasselbe System beschreiben (Annahme 5); weichen die beiden Zählungen stark voneinander ab, wird das als eigene Warnung ausgewiesen statt stillschweigend verrechnet.
- **Bewusst nicht:** keine Aufschlüsselung je Wertstrom und keine Warteschlangensicht. Die Bestandszeitreihe wird **je Stage** gebildet und mitgeführt, damit DR1/DR3 (Reinertsen: Warteschlangen-Sicht, High-Queue-State-Anteil) darauf aufsetzen können, ohne die Datenform noch einmal zu ändern — dargestellt wird vorerst nur die Summe.
- **Aufwand:** M.

---

### A7 · Zykluszeit-Grenzen im Diagrammkopf benennen · ✅ umgesetzt (10.09.2026)
- **Was:** Über den Kennzahlen des Flow-Time-Diagramms steht jetzt eine Zeile, die sagt, **wo die Uhr läuft**: bei Methode A vom ersten Eintritt in die `<First>`-Stage bis zum letzten Eintritt in die `<Closed>`-Stage, bei Methode B als Summe der Verweilzeiten aller Stages vor der `<Closed>`-Stage — dort ausdrücklich **ohne Startgrenze**. Ist eine Grenze nicht deklariert (kein `--workflow`), steht das da statt einer geratenen Stage. Zusätzlich zählt der Bericht aus der Transitions-Datei aus, wie viele Vorgänge eine der beiden Grenzstages nie betreten haben und deshalb mit einer **abgeleiteten Grenze** in die Rechnung eingehen — am Start wie am Ende, denn die `<Closed>`-Stage ist nicht zwingend die letzte im Workflow; steht `Done` oder `Monitoring` dahinter, kann auch der Abschluss übersprungen werden. Jeder Fund steht zusätzlich als eigene Warnung auf der Konsole.
...
- **Bewusst nicht:** `L1` (DoD-Deklaration je Quelle) ist Phase B und braucht ein Konfigurationsfeld, das es nicht gibt; `L3` (Zweck und Urheber neben der Zahl) setzt ein Metrik-Definitionsblatt voraus, das ebenfalls fehlt — beide sind eigene Vorgänge. Die `<Closed>`-Ersatzregel wird **mit** ausgewiesen (Hinweis Robert, 10.09.2026: „es kann auch nach dem schließenden Status noch weitere Status geben, deswegen kann auch der schließende übersprungen werden“) — in `ART_E` betrifft das fünf Vorgänge.
- **Aufwand:** S.
...
### D9 · Kennzeichnung folgt der Adresse, nicht dem Anbieternamen · 🔎 **Entscheidung offen**
*(aufgefallen beim Bau der Adressfelder, 10.09.2026)*
- **Was ist der Befund:** Seit v0.30.0 lässt sich die Ollama-Adresse setzen — GUI-Feld *Adresse*, `--base-url` / `--llm-base-url`, Umgebungsvariable `OLLAMA_HOST`. Zeigt sie auf einen anderen Rechner, verlassen die Daten den eigenen. **Die Kennzeichnung merkt das nicht.** `deployment_class` ist ein festes Klassenattribut des Providers (`llm/providers/ollama.py`: `DEPLOYMENT_LOCAL`) und hängt am Anbieternamen, nicht an der Adresse. Folge: Der Art.-50-Banner schreibt weiterhin `… @ local` (`llm/guard.py:122`), und der Betreiber-Nachweis (`llm/audit.py`) protokolliert `deployment_class: "local"` — **die Adresse steht dort überhaupt nicht**.
- **Warum das zählt:** Der ganze Sinn der Kennzeichnung ist die Aussage, **wohin die Daten gehen**. Eine Zeile, die „lokal“ behauptet, während die Anfrage an einen fremden Host geht, ist schlechter als gar keine — sie beruhigt falsch. Die rechtliche Leitplanke (c) dieses Abschnitts verlangt ausdrücklich, dass „die Deployment-Entscheidung Teil der Konfiguration“ ist und protokolliert wird; genau das ist sie hier nicht mehr, seit die Adresse konfigurierbar wurde.
- **Zu entscheiden — drei Fragen:**
  1. **Wonach richtet sich die Klasse?** Vorschlag: Zeigt die aufgelöste Adresse auf `localhost` / `127.0.0.1` / `::1`, bleibt es `local`; sonst eine eigene Klasse. Die Grenze ist nicht trivial — ein Rechner im eigenen Netz ist etwas anderes als ein Dienst im Internet, aber von außen sind beide nur „nicht dieser Rechner“.
  2. **Wie heißt die neue Klasse?** `external_api` wäre irreführend (das ist Claude, mit Token und Anbietervertrag). Ein eigener Wert wie `remote_local` oder `self_hosted_remote` beschreibt es genauer — aber jeder neue Wert taucht im Banner auf, den Kundinnen und Kunden lesen.
  3. **Gehört die Adresse in den Betreiber-Nachweis?** Dafür: Ohne sie lässt sich im Nachhinein nicht belegen, wohin ein Entwurf ging. Dagegen: Ein Hostname kann selbst eine Auskunft über die Infrastruktur sein. Möglicher Mittelweg — nur die Klasse und ein Hash der Adresse, wie es die Datei mit Ein- und Ausgabe schon hält.
- **Wo:** `llm/base.py` (`LlmProvider`-Contract: `deployment_class` ist heute ein Attribut, müsste je Aufruf entschieden werden), `llm/providers/ollama.py` (`resolve_base_url` weiß die Antwort bereits), `llm/guard.py` (Banner), `llm/audit.py` (Nachweis). Der Contract zu ändern trifft alle Provider — vor dem Bauen klären, ob `complete()` die Klasse im `LlmResult` selbst setzt (dann bleibt der Contract schmal) statt als Klassenattribut.
- **Aufwand:** S im Code, die Entscheidung ist der Aufwand. **Vor dem nächsten Kundeneinsatz mit entferntem Ollama zu klären.**
...
- **(c) Betreiber-Nachweis.** Modell, Version, Systemvorgaben und Datenklassifizierung je Deployment werden protokolliert (Grundlage für Kompetenz- und Transparenzpflicht, Art. 4/Art. 50); die Deployment-Entscheidung (on-prem / EU-Cloud / Zero-Retention) ist Teil der Konfiguration, nicht der Nutzerwahl. ⚠️ **Seit v0.30.0 nicht mehr durchgehalten:** Die Ollama-Adresse ist konfigurierbar, die Kennzeichnung folgt ihr aber nicht — siehe **D9**.
