...
def content_de(st: dict) -> list:
    ...
    story += [PageBreak(),
              ...
              PRE("python -m testdata_generator --scenario portfolio \\\n"
                  "    --output demo/ --seed 42 --scale m", st),
              P("<b>Skalen (--scale, auch als Auswahl in der GUI):</b> Die neun "
                "Register je Solution bestehen aus festen Story-Ankern plus "
                "einer seed-generierten Grundmenge — <b>s</b> = nur Anker "
                "(heutige Erzähldichte), <b>m</b> = realistische Demo "
                "(Standard), <b>l</b> = Stresstest (dichte Heatmap, volle "
                "Roadmap-Matrix). Die Anker und ihre Geschichten sind auf "
                "jeder Skala identisch; die Grundmenge hält Invarianten ein, "
...
def content_en(st: dict) -> list:
    ...
    story += [PageBreak(),
              ...
              PRE("python -m testdata_generator --scenario portfolio \\\n"
                  "    --output demo/ --seed 42 --scale m", st),
              P("<b>Scales (--scale, also a picker in the GUI):</b> the nine "
                "registers per solution consist of fixed story anchors plus "
                "a seed-generated base population — <b>s</b> = anchors only "
                "(today's storytelling density), <b>m</b> = realistic demo "
                "(default), <b>l</b> = stress test (dense heatmap, full "
                "roadmap matrix). Anchors and their stories are identical "
...
def content_ro(st: dict) -> list:
    ...
    story += [PageBreak(),
              ...
              PRE("python -m testdata_generator --scenario portfolio \\\n"
                  "    --output demo/ --seed 42 --scale m", st),
              P("<b>Scari (--scale, si ca selector in GUI):</b> cele noua "
                "registre per solutie constau din ancore de poveste fixe "
                "plus o populatie de baza generata cu seed — <b>s</b> = doar "
                "ancore, <b>m</b> = demo realist (implicit), <b>l</b> = test "
                "de stres (heatmap densa, matrice roadmap plina). Ancorele "
                "si povestile lor sunt identice la orice scara; baza "
...
def content_pt(st: dict) -> list:
    ...
    story += [PageBreak(),
              ...
              PRE("python -m testdata_generator --scenario portfolio \\\n"
                  "    --output demo/ --seed 42 --scale m", st),
              P("<b>Escalas (--scale, tambem como seletor na GUI):</b> os "
                "nove registos por solution consistem em ancoras de "
                "historia fixas mais uma populacao base gerada com seed — "
                "<b>s</b> = so ancoras, <b>m</b> = demo realista "
                "(predefinicao), <b>l</b> = teste de stress (heatmap densa, "
                "matriz roadmap cheia). As ancoras e as suas historias sao "
...
def content_fr(st: dict) -> list:
    ...
    story += [PageBreak(),
              ...
              PRE("python -m testdata_generator --scenario portfolio \\\n"
                  "    --output demo/ --seed 42 --scale m", st),
              P("<b>Echelles (--scale, aussi un selecteur dans la GUI) :</b> "
                "les neuf registres par solution se composent d'ancres "
                "d'histoire fixes plus une population de base generee par "
                "seed — <b>s</b> = ancres seules, <b>m</b> = demo realiste "
                "(defaut), <b>l</b> = test de charge (heatmap dense, matrice "
                "roadmap pleine). Les ancres et leurs histoires sont "
                "identiques a chaque echelle ; la base respecte des "
