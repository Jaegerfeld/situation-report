    ...
    story += [
        ...
        *_gui_figure(st, "Abb. 1: Die Oberfläche von Solutions &amp; Portfolios."),
        ...
        _P("<font name='Courier'>--narrate [PROVIDER]</font> ergänzt das "
           ...
           "<font name='Courier'>--llm-model</font>, <font name='Courier'>"
           "--llm-base-url</font> und <font "
           ...
           "dem Delta-Briefing-Knopf. <b>Liegen mehrere Modelle in "
           "Ollama</b>, wählt das Feld <i>Modell</i> daneben aus; es "
           "schlägt die installierten vor, lässt sich aber auch frei "
           "beschriften, und leer heißt: der Anbieter entscheidet. "
           "Läuft Ollama auf einem anderen Rechner, nimmt das Feld "
           "<i>Adresse</i> sie auf (sonst gilt die Umgebungsvariable "
           "OLLAMA_HOST, sonst http://localhost:11434). Anbieter, Modell "
           "und Adresse gehören zu <b>diesem Rechner</b>: Sie werden "
           "für den nächsten Start gemerkt, wandern aber "
           "ausdrücklich NICHT in die gespeicherte "
           "Solution-Konfiguration, die man weitergibt. Welche Modelle "
           "installiert sind, zeigt <font name='Courier'>python -m llm "
           "models</font>. <b>Wo der Haken NICHT wirkt:</b> bei "
           "„Konferenzmappe …“ — die ist absichtlich "
           "deterministisch, weil sie die Unterlage einer bereits "
           "einberufenen Konferenz ist — und bei PDF-Ausgabe, die "
           "keinen Ort für den redigierbaren Entwurf hat. In beiden "
           "Fällen sagt die Statuszeile das jetzt, statt den Haken "
           "wirkungslos zu übergehen. Ollama einrichten: separate "
    ...
    story += [
        ...
        *_gui_figure(st, "Fig. 1: The Solutions &amp; Portfolios interface."),
        ...
        _P("<font name='Courier'>--narrate [PROVIDER]</font> adds the "
           ...
           "name='Courier'>--llm-model</font>, <font name='Courier'>"
           "--llm-base-url</font> and <font name='Courier'>"
           ...
           "the delta-briefing button. <b>With more than one model in "
           "Ollama</b>, the <i>Model</i> field beside it picks one; it "
           "offers the installed models but stays typable, and empty "
           "means: let the provider decide. If Ollama runs on another "
           "machine, the <i>Address</i> field takes it (otherwise the "
           "OLLAMA_HOST environment variable applies, else "
           "http://localhost:11434). Provider, model and address belong to "
           "<b>this machine</b>: they are remembered for the next start "
           "but deliberately do NOT go into the saved solution "
           "configuration you hand on. <font name='Courier'>python -m llm "
           "models</font> lists what is installed. <b>Where the checkbox does "
           "NOT apply:</b> 'Conference pre-read …' — deliberately "
           "deterministic, being the paper for an already convened "
           "conference — and PDF output, which has nowhere to put the "
           "editable draft. In both cases the status line now says so "
           "instead of ignoring the checkbox silently. "
           "Setting up Ollama: separate guide "
    ...
    story += [
        ...
        *_gui_figure(st, "Fig. 1: Interfata Solutions &amp; Portfolios."),
        ...
        _P("<font name='Courier'>--narrate [PROVIDER]</font> adauga la "
           ...
           "</font>, <font name='Courier'>--llm-base-url</font> si <font "
           "name='Courier'>--llm-lang de|en</font> rafineaza. Trei lucruri sunt cablate si nu pot fi ocolite: "
           ...
           "providerului, campul <i>Model</i> si campul <i>Adresa</i>. "
           "Campul Model propune modelele instalate in Ollama, dar accepta "
           "si un nume scris manual; gol inseamna valoarea implicita a "
           "furnizorului. Adresa este necesara doar daca Ollama ruleaza pe "
           "alt calculator (altfel OLLAMA_HOST, apoi "
           "http://localhost:11434). Ambele apartin ACESTUI calculator si "
           "nu intra in configuratia salvata a solutiei. Instalarea Ollama: ghid separat "
    ...
    story += [
        ...
        *_gui_figure(st, "Fig. 1: A interface Solutions &amp; Portfolios."),
        ...
        _P("<font name='Courier'>--narrate [PROVIDER]</font> acrescenta ao "
           ...
           "</font>, <font name='Courier'>--llm-base-url</font> e <font "
           "name='Courier'>--llm-lang de|en</font> refinam. Tres coisas estao cabladas e nao podem ser "
           ...
           "mais a escolha do provider, o campo <i>Modelo</i> e o campo "
           "<i>Endereco</i>. O campo Modelo propoe os modelos instalados "
           "no Ollama mas aceita um nome escrito; vazio significa a "
           "predefinicao do fornecedor. O Endereco so e preciso se o "
           "Ollama correr noutra maquina (senao OLLAMA_HOST, depois "
           "http://localhost:11434). Ambos pertencem a ESTA maquina e nao "
           "entram na configuracao guardada da solution. Instalar o Ollama: guia separado "
    ...
    story += [
        ...
        *_gui_figure(st, "Fig. 1 : L'interface Solutions &amp; Portfolios."),
        ...
        _P("<font name='Courier'>--narrate [PROVIDER]</font> ajoute au "
           ...
           "--llm-model</font>, <font name='Courier'>--llm-base-url</font> "
           "et <font name='Courier'>--llm-lang de|en"
           ...
           "case 'Narration IA (brouillon)' plus le choix du provider, le "
           "champ <i>Modele</i> et le champ <i>Adresse</i>. Le champ "
           "Modele propose les modeles installes dans Ollama mais accepte "
           "aussi un nom saisi ; vide signifie la valeur par defaut du "
           "fournisseur. L'Adresse n'est utile que si Ollama tourne sur "
           "une autre machine (sinon OLLAMA_HOST, puis "
           "http://localhost:11434). Les deux appartiennent a CETTE "
           "machine et n'entrent pas dans la configuration enregistree de "
           "la solution. "
