...
Status groups categorise workflow stages into meaningful phases that indicate where an issue currently stands in the delivery process.

The system currently defines three groups. The architecture is designed to allow additional groups to be added in future versions.

| Group | Meaning |
|-------|---------|
| `To Do` | The issue has not yet been actively started |
| `In Progress` | The issue is being actively worked on |
| `Done` | The issue is complete |

### Stage-level classification

The function `classify_stages()` assigns each stage of the workflow to a group based on its **position relative to two boundary stages**:

- **First Stage** (`first_stage`): The stage at which active work begins. It marks the boundary between `To Do` and `In Progress`.
- **Closed Stage** (`closed_stage`): The stage that marks an issue as complete. It marks the boundary between `In Progress` and `Done`.
...
Groups:  To Do      To Do         In Progress  In Progress  Done    Done
```
...
| Situation | Behaviour |
|-----------|-----------|
| `first_stage` unknown or not in workflow | All stages → `In Progress` |
| `closed_stage` unknown or not in workflow | All stages from `first_stage` onward → `In Progress` (no `Done` boundary) |
| Empty workflow | No assignment (empty result) |

### Issue-level classification

For individual issues the status group is derived not from the stage position but from the issue's **date fields** (`issue_stage_group()`):

| Condition | Status group |
|-----------|-------------|
| `Closed Date` is set | `Done` |
| Only `First Date` is set (not yet closed) | `In Progress` |
| Neither `First Date` nor `Closed Date` is set | `To Do` |

`Closed Date` takes precedence: an issue is considered `Done` as soon as it has a closing date, regardless of whether `First Date` is set.

This logic is applied during the **Report Excel export**: each issue receives a `Status Group` column.
...
| Column | Content |
|--------|---------|
| `Status Group` | Status group of the issue (`To Do` / `In Progress` / `Done`) — see [Status Groups](#status-groups) |
