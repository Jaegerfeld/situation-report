...
Statusgruppen fassen Workflow-Stages zu fachlichen Kategorien zusammen und geben Auskunft darüber, in welcher Bearbeitungsphase sich ein Issue befindet.

Das System kennt derzeit drei Gruppen. Die Architektur ist so ausgelegt, dass weitere Gruppen in zukünftigen Versionen ergänzt werden können.

| Gruppe | Bedeutung |
|--------|-----------|
| `To Do` | Das Issue wurde noch nicht aktiv begonnen |
| `In Progress` | Das Issue befindet sich in aktiver Bearbeitung |
| `Done` | Das Issue ist abgeschlossen |

### Klassifizierung von Stages

Die Funktion `classify_stages()` ordnet jede Stage des Workflows anhand ihrer **Position relativ zu zwei Grenz-Stages** einer Gruppe zu:

- **First Stage** (`first_stage`): Die Stage, ab der aktive Arbeit beginnt. Sie markiert die Grenze zwischen `To Do` und `In Progress`.
- **Closed Stage** (`closed_stage`): Die Stage, die den Abschluss eines Issues markiert. Sie markiert die Grenze zwischen `In Progress` und `Done`.

**Beispiel:**

```
Stages:   [Backlog]  [Refinement]  [In Dev]  [Review]  [Closed]  [Archive]
                                   ↑                   ↑
                               first_stage         closed_stage

Gruppen:  To Do      To Do         In Progress  In Progress  Done    Done
```
...
| Situation | Verhalten |
|-----------|-----------|
| `first_stage` unbekannt oder nicht im Workflow | Alle Stages → `In Progress` |
| `closed_stage` unbekannt oder nicht im Workflow | Alle Stages ab `first_stage` → `In Progress` (keine `Done`-Grenze) |
| Leerer Workflow | Keine Zuordnung (leeres Ergebnis) |

### Klassifizierung von Issues

Für einzelne Issues wird die Statusgruppe nicht aus der Stage-Position, sondern aus den **Datumsfeldern** des Issues abgeleitet (`issue_stage_group()`):

| Bedingung | Statusgruppe |
|-----------|-------------|
| `Closed Date` vorhanden | `Done` |
| Nur `First Date` vorhanden (noch nicht geschlossen) | `In Progress` |
| Weder `First Date` noch `Closed Date` | `To Do` |

`Closed Date` hat dabei Vorrang: ein Issue gilt als `Done`, sobald es ein Abschlussdatum hat — unabhängig davon, ob ein `First Date` gesetzt ist.

Diese Logik wird beim **Report-Excel-Export** verwendet: Jedes Issue erhält dort eine `Status Group`-Spalte.
...
| Spalte | Inhalt |
|--------|--------|
| `Status Group` | Statusgruppe des Issues (`To Do` / `In Progress` / `Done`) — siehe [Statusgruppen](#statusgruppen) |
